...
func RunCPUAES(ctx context.Context, dur time.Duration, threads, keyLen int) Result {
	if threads <= 0 {
	...
	wg.Wait()
	return Result{Name: "AES-CTR", Threads: threads, Duration: dur, Bytes: bytes, Unit: "B/s", Notes: "key=" + fmt.Sprintf("%d-bit", keyLen*8)}
}
